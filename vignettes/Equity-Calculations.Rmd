...
The funciton tells us that the error is that the column names of both data frames do not match.

Equally, the names of the columns need to agree, not only in number (`eq_2` had one column more than `eq`) but also in the exact wording. For example, the names of the questions are the following:
